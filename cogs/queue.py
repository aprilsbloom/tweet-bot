...
class Queue(commands.Cog):
	...
	group = discord.app_commands.Group(
		name = "queue",
		description = 'Modify or view the queue'
	)

	# Queue viewing
	@group.command(
		name = "view",
		description = "View items in the queue"
	)
	async def queue_view(self, interaction: discord.Interaction):
		...
		if post_queue_length > 1:
			...
			for count, post in enumerate(config["twitter"]["post_queue"], start=1):
				...
				embed.set_footer(text = f"Post {count + 1} / {post_queue_length}")
