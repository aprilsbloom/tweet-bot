...
class EditModal(discord.ui.Modal):
    def __init__(self, post, title="Edit post"):
        ...
        self.add_item(
            discord.ui.TextInput(label="Gif URL", default=self.post["catbox_url"])
        )
...
class Queue(commands.Cog):
    ...
    @discord.app_commands.command(
        name="queue", description="View / remove items from the tweet queue"
    )
        ...
        if cmd_choice.value == "view":
            if post_queue_length > 1:
                ...
                for post in config["twitter"]["post_queue"]:
                    ...
                    if post["caption"] != "":
                        embed.add_field(
                            name="Caption", value=post["caption"], inline=False
                        )

                    embed.add_field(
                        name="Alt text", value=post["alt_text"], inline=False
                    )
